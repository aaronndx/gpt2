...
class FineWebDatasetProcessor:
    """
    A class to handle the downloading, tokenizing, and writing-to-disk of the FineWeb dataset.
    """
    def __init__(self):
        """Initializes the tokenizer."""
        self.enc = tiktoken.get_encoding("gpt2")
        self.eot = self.enc._special_tokens['<|endoftext|>'] # 50256
    
    def _tokenize_worker(self, doc):
        """Tokenizes a single document and returns a numpy array of uint16 tokens."""
        tokens = [self.eot] # <|endoftext|> is delimiter between documents. Adds one as the start too.
    ...
    def tokenize_and_save(self, local_dir="edu_fineweb10B", target_subset="sample-10BT", shard_size=int(1e8)):
        ...
        try:
            # This will work when running as a .py script
            # It creates the directory relative to the script's location
            DATA_CACHE_DIR = os.path.join(os.path.dirname(__file__), local_dir)
            print(f"Running as a script, setting cache to: {DATA_CACHE_DIR}")
        except NameError:
            # This will be executed in an interactive environment like Colab or Jupyter
            # where __file__ is not defined. It uses the current working directory.
            DATA_CACHE_DIR = local_dir
            print(f"Running in an interactive session, setting cache to: {DATA_CACHE_DIR}")
        ...
        with mp.Pool(n_procs) as pool:
            ...
            token_count = 0
            progress_bar = None
            for tokens in pool.imap(self._tokenize_worker, ds, chunksize=16):
...
if __name__ == "__main__":
    processor = FineWebDatasetProcessor()
    processor.tokenize_and_save()